...
To inspect the performance of the contig score components, we used the smallest of the assemblies from real data, the Trinity assembly of yeast.

The contigs were aligned to the reference transcriptome and the reference genome with blastn, and the assigned read alignments output by Salmon were used to calculate per-bases coverage using samtools mpileup.

Although contigs that score poorly on any given score component are likely to contain assembly problems, it does not follow that contigs scoring well on any given score componenet are well-assembled, as they often score poorly on other components. Thus, the lowest scoring set of contigs are more informative than the highest scoring, but we include both for completeness.
...
plot_blasthits <- function(dt, dir, expand_hits=FALSE) {
...
plot_hits <- function(blasthits, dir) {
  dir.create(dir)
  group_by(blasthits, contig_name) %>%
    do(plot_blasthits(., dir))
}
...
$s(C_{nuc})$ measures how well the read evidence supports the identity of each base in a contig. A low score might indicate family collapse (Figure 1), thus we visualised reference genes hitting each contig for the lowest and highest scoring 30 contigs according to this score component, vertically stacking the alignments to highlight cases where multiple genes hit a single contig. The number of visualised hits was limited to 3 per contig.
...
```{r p_seq_true_hi, echo = FALSE, results = "asis", warning=FALSE, message=FALSE}
setwd('score_component_examination')
...
plot_expanded_hits(blasthits, 'p_seq_true_hi')
print_results(top30, 'p_seq_true', 'p_seq_true_hi')
```

## $s(C_{seg})$

$s(C_{seg})$ estimates the probability that the coverage along the contig is best explained by a single Dirichlet distribution, as opposed to two or more. A low score might indicate chimerism (Figure 1), therefore we visualised reference gene hits on a single plane, to highlight cases where hits against different genes are non-overlapping, indicating a chimera. Each reference gene is assigned a single colour.
...
```{r p_not_segmented_hi, echo = FALSE, results = "asis", warning=FALSE, message=FALSE}
setwd('score_component_examination')
...
plot_hits(blasthits, 'p_not_segmented_hi')
print_results(top30, 'p_not_segmented', 'p_not_segmented_hi')
```

## $s(C_{ord})$

$s(C_{seg})$ measures the structural correctness of the contig. A low score might indicate misassembly (Figure 1). We visualised reference hits on a single plane, to highlight cases where transcripts are poorly covered by reference hits. Each reference gene is assigned a single colour.
...
```{r p_good_hi, echo = FALSE, results = "asis", warning=FALSE, message=FALSE}
setwd('score_component_examination')
...
plot_hits(blasthits, 'p_good_hi')
print_results(top30, 'p_good', 'p_good_hi')
```

## $s(C_{cov})$

$s(C_{seg})$ measures the proportion of bases that have read support. We therefore visualised whether each base was covered by at least one supporting read.
...
plot_hits(cov_rle, 'p_bases_covered_lo')
print_results(top30, 'p_bases_covered', 'p_bases_covered_lo')
```

## 30 highest scoring

```{r p_bases_covered_hi, echo = FALSE, results = "asis", warning=FALSE, message=FALSE}
setwd('score_component_examination')
...
## write to file and grep out coverage
if (!file.exists('p_bases_covered_hi_cov.txt')) {
  grep_names <- sapply(top30$contig_name, function(x) {
    paste(x, '\\s', sep="")
  })
  file.remove("p_bases_covered_hi_ids.txt")
  lapply(grep_names, write, "p_bases_covered_hi_ids.txt", append=TRUE, ncolumns=1)
  system("grep -f p_bases_covered_hi_ids.txt ../assigned.coverage.count > p_bases_covered_hi_cov.txt")
}

cov_rle <- NULL
if (file.info('p_bases_covered_hi_cov.txt')$size == 0) {
  ## no hits, make blanks to illustrate by reusing blank from last metric
  cov_rle <- blasthits[rep(1, 30),]
  cov_rle$contig_name <- top30$contig_name
  cov_rle$q_start <- 0
  ...
  # hits
  cov <- fread("p_bases_covered_hi_cov.txt")
  setnames(cov, c('contig_name', 'pos', 'cov'))
  cov_res <- merge(cov, yeast_trin, by='contig_name')
  cov_rle <- group_by(cov_res, contig_name) %>%
    do(coverage_rle(.))
...
plot_hits(cov_rle, 'p_bases_covered_hi')
print_results(top30, 'p_bases_covered', 'p_bases_covered_hi')
```
