...
We sought to ensure whether each of the contig score components was contributing useful information. Figure 2a shows that all the components were distributed across the full range (0 to 1) in all assemblies, and that each exhibits enrichment towards the extremes. This suggests again that the components capture real variability present in assemblies, and that the components are useful for classification.

In order to determine whether the all the components were necessary, we examined correlation between the components. To avoid giving preference to larger assemblies, we sampled 5000 contigs from each assembly. Pearson correlation was calculated between each pair of score components (Figure 2b) demonstrate that the score components are independent, i.e. that each captures unique information compared to the other components.
...
As expected, we found that the higher transrate score deciles tend to contain higher proportions of 'true' contigs when compared to the reference (figure 4a). Across rice, mouse and human there was a very clear positive trend with increasing proportions of true contigs in increasing score deciles. The trend was present by less clear in yeast.
...
To examine the hypothesis that the ~30% of contigs in the highest score bins in the analysis based on real data were caused by incomplete reference annotation, we conducted an experiment using simulated reads where we knew the exact set of possible transcripts that the assembly might reconstruct.
...
- With some significant manipulation they can be turned into a sort of contig score, specifically if I take $10^{abs(contig_impact_score)/max(contig_impact_score)}$, I get a value between 0 and 1 that can be used to classify contigs
- intially I thought the most fair thing would be to compare to this and I did some test analysis. We generally compare very favourably to the RSEM-eval-based score on accuracy - we are much better for 3/4 assemblies, and they are a tiny bit better for one assembly.
- however, I am now thinking that we're doing their work for them - if they wanted to make a classification score they could do that. Maybe we should just compare at the assembly score level, and focus on accuracy of the transrate contig score here?
...
- from each test species, we take one chromosome and simulate a small set of rnaseq reads from it

then:

1. we take those reads a do a parameter sweep with 3 different assemblers, say 40 assemblies from each
...
- take the assemblies and throw out the contigs with bad scores, show that the assmebly score gets better
- show that there's an optimal cutoff? we could save this for the transfix paper
...
To provide examples of real datasets and scores we downloaded all transcriptomes available on the NCBI Transcriptome Shotgun Assembly database (TSA). Assemblies from this database were selected for further analysis only if the following criteria were met:
